...
from tensorflow.keras.layers import Conv2D, Add, ZeroPadding2D, MaxPooling2D, AveragePooling2D, GaussianNoise
...
from CNN.layers import UpsampleLike
from CNN.submodels import default_classification_model,default_regression_model , default_submodels, __build_model_pyramid , __build_pyramid
import tensorflow
...
class Network():
    ...
    def _create_model(self):
        ...
        conv4_x = self._resnet_body(
            ...
            # stage 4
        conv4_x_x = self._resnet_body(
            x=conv4_x, filters_list=filters_list[3], num_blocks=blocks_list[3], strides=2, name='res5')

        # stage 5
        conv5_x = self._detnet_body(
            x=conv4_x_x, filters_list=filters_list[4], num_blocks=blocks_list[4], strides=1, name='dires5')
        ...
        tensorflow.keras.utils.plot_model(model , "detnet.png", show_shapes=True)
...
def create_final_model(nb_of_classes):
    ...
    layer_outputs=[
        ...
        detnet.get_layer('res4_6_2_relu').output,
        detnet.get_layer('res3_4_relu').output,
        detnet.get_layer('res2_3_relu').output]
    ...
    retina = retinanet(inputs, backbone_layers, nb_of_classes)
    
    #tensorflow.keras.utils.plot_model(retina, 'retina.png', show_shapes=True)

    return retina

#retina.save('retina.h5')

retina = create_final_model(2)
