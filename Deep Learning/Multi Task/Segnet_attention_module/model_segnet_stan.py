...
class SegNet(nn.Module):
    def __init__(self):
        ...
        #self.up_sampling = nn.UpsamplingBilinear2d()
    ...
    def forward(self, x):
        ...
        for i in range(5):
            if i == 0:
                ...
                g_upsampl[i] = nn.Upsample( scale_factor=2)(g_maxpool[-1])

                g_decoder[i][0] = self.decoder_block[-i - 1](g_upsampl[i])
                g_decoder[i][1] = self.conv_block_dec[-i - 1](g_decoder[i][0])
            else:
                #g_upsampl[i] = self.up_sampling(g_decoder[i - 1][-1], indices[-i - 1])
                g_upsampl[i] =nn.Upsample( scale_factor=2)(g_decoder[i - 1][-1])
        ...
        for i in range(1):
            ...
            for j in range(5):
                if j == 0:
                    atten_decoder[i][j][0] = nn.Upsample( scale_factor=2)(atten_encoder[i][-1][-1])
                    atten_decoder[i][j][0] = self.decoder_block_att[-j - 1](atten_decoder[i][j][0])
                    atten_decoder[i][j][1] = self.decoder_att[i][-j - 1](torch.cat((g_upsampl[j], atten_decoder[i][j][0]), dim=1))
                    atten_decoder[i][j][2] = (atten_decoder[i][j][1]) * g_decoder[j][-1]
                else:
                    atten_decoder[i][j][0] = nn.Upsample( scale_factor=2)(atten_decoder[i][j - 1][2])
                    atten_decoder[i][j][0] = self.decoder_block_att[-j - 1](atten_decoder[i][j][0])
                    atten_decoder[i][j][1] = self.decoder_att[i][-j - 1](torch.cat((g_upsampl[j], atten_decoder[i][j][0]), dim=1))
                    atten_decoder[i][j][2] = (atten_decoder[i][j][1]) * g_decoder[j][-1]
