...
def procenteVenit():
    ...
    #w1 = np.array([0.094])
    w2=np.arange(0.02,0.05, 0.002)
    w3=np.arange(0.02, 0.04, 0.002)
    weights = [w1,w2,w3]
    listWeights =[]
...
def procenteCheltuieli():
    w1=np.array([0.02])
    w2=np.arange(0.02, 0.04, 0.003)
    w3=np.arange(0.02, 0.03,0.003)
    weights = [w1,w2,w3]
    listWeights =[]
    for i in itertools.product(*weights):
        listWeights.append(i)
            ...
            try:
                ...
                if i %50==0:
                    ...
                    if ((Flux2<Flux3<Flux4) and (Flux2>0) and (Flux3>0) and (Flux4>0)):
                        ...
                        if (RentabilitateInvestitie*100<=4 and RentabilitateInvestitie*100>=0):
                            workbook.save(originalPathNew)
                            workbook.close()
                        
                        if (RentabilitateInvestitie*100<=4 and RentabilitateInvestitie*100>=1.5): 
                            workbook.save(originalPathNew)
                            workbook.close()
